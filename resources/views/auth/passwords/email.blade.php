...
@section('title', __('auth.forgot_password_page_title'))
@section('auth_nav_subtitle', 'Password assistance')
...
@section('auth_hero')
    ...
    <div class="mt-10 space-y-3">
        <div class="auth-chip">
            ...
            <span class="text-sm font-semibold">{{ __('auth.send_reset_link') }}</span>
        </div>
        <div class="auth-chip">
...
@section('auth_panel')
    <div class="flex items-start justify-between gap-4">
        <div>
            ...
            <h2 class="auth-heading mt-2 text-3xl font-semibold tracking-[-0.04em] text-slate-950">{{ __('auth.forgot_password') }}</h2>
            <p class="mt-2 text-sm leading-6 text-slate-500">{{ __('auth.forgot_password_subtitle') }}</p>
        </div>
    ...
    <form method="POST" action="{{ route('password.email', ['locale' => app()->getLocale()]) }}" class="mt-6 space-y-5">
        @csrf
