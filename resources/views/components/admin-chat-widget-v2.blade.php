        ...
        <div class="chat-premium-body flex min-h-0 flex-1 flex-col">
            <div id="admin-list-view-v2" class="flex min-h-0 flex-1 flex-col">
                <div class="shrink-0 border-b border-slate-200/70 bg-white/80 px-4 py-4 sm:px-5">
                    <div class="mb-3">
                        <p class="text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-500">{{ __('admin_chat.select_user') }}</p>
                        <h4 class="mt-2 text-sm font-semibold text-slate-900">{{ __('admin_chat.title') }}</h4>
                        <p class="mt-1 text-xs leading-5 text-slate-500">{{ __('admin_chat.recipient_hint') }}</p>
                    </div>
                    <label for="admin-user-search-v2" class="mb-2 block text-xs font-semibold uppercase tracking-[0.14em] text-slate-500">{{ __('admin_chat.select_user') }}</label>
                    <input id="admin-user-search-v2" type="text" class="chat-premium-search" placeholder="{{ __('admin_chat.select_user_placeholder') }}">
                ...
                <div id="conversations-items-v2" class="chat-premium-scroll min-h-0 flex-1 px-4 py-4 sm:px-5">
                    <div class="chat-premium-empty">
                        ...
                        <p class="text-sm font-semibold text-slate-800">{{ __('admin_chat.loading_conversations') }}</p>
                        <p class="text-xs text-slate-500">{{ __('admin_chat.recipient_hint') }}</p>
                    </div>
                </div>
            </div>
            ...
            <div id="admin-thread-view-v2" class="hidden min-h-0 flex-1 flex-col">
                ...
                <div id="admin-thread-typing-wrap-v2" class="hidden px-4 pb-3 sm:px-5">
                    <div class="chat-premium-typing">
                        ...
                        <span id="admin-thread-typing-text-v2" class="text-xs font-medium">{{ __('admin_chat.typing') }}</span>
                    </div>
                </div>
                ...
                <div class="chat-premium-composer shrink-0">
                    ...
                    <div class="chat-premium-composer-row">
                        ...
                        <button type="button" id="admin-file-trigger-v2" class="chat-premium-icon-button" aria-label="{{ __('chat.attach_file') }}">
                            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13"></path>
                            </svg>
                        </button>
                        <textarea id="admin-chat-input-v2" rows="1" class="chat-premium-input" placeholder="{{ __('admin_chat.reply_placeholder') }}"></textarea>
...
(() => {
    ...
    async function loadUsers(search = '') {
        const url = search ? `{{ route("chat.users") }}?q=${encodeURIComponent(search)}` : '{{ route("chat.users") }}';
        try {
    ...
    async function loadThread() {
        ...
        try {
            const { response, data } = await json(`{{ route("chat.messages") }}/${state.activeUser.id}`);
            if (!response.ok || !data || !data.success) {
    ...
    async function sendMessage() {
        ...
        try {
            const { response, data } = await json('{{ route("chat.send") }}', {
                method: 'POST',
