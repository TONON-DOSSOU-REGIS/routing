...
<style>
    ...
    #{{ $trackerId }} .market-tab {
        background: rgba(148, 163, 184, 0.12);
        color: #cbd5e1;
        transition: background-color 180ms ease, color 180ms ease;
    }

    #{{ $trackerId }} .market-tab.active {
        background: #f8fafc;
        color: #0f172a;
    }

    #{{ $trackerId }} .market-card {
    ...
    #{{ $trackerId }}:not(.market-tracker-widget--compact) .market-card:hover {
        transform: translateY(-3px);
        border-color: rgba(191, 219, 254, 0.3);
    }

    #{{ $trackerId }} .market-label {
        color: #f8fafc;
    }

    #{{ $trackerId }} .market-sub {
        color: #94a3b8;
    }

    #{{ $trackerId }} .market-price {
        color: #f8fafc;
    }

    #{{ $trackerId }} .market-muted {
        color: #94a3b8;
    }
</style>

<div id="{{ $trackerId }}" class="market-tracker-widget {{ $compact ? 'market-tracker-widget--compact' : '' }}" data-compact="{{ $compact ? '1' : '0' }}">
    <div class="mb-5 flex flex-wrap items-start justify-between gap-4">
        <div class="flex min-w-0 items-center gap-3">
            ...
            <div class="min-w-0">
                <h3 class="truncate text-lg font-bold text-white sm:text-xl">{{ __('market.title_tracker') }}</h3>
                <p class="market-muted truncate text-sm" data-market-last-update>{{ __('market.last_update_now') }}</p>
            </div>
        </div>

        <button type="button" data-market-refresh class="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white/10 transition-colors duration-200 hover:bg-white/20" title="{{ __('market.refresh') }}">
            <svg class="h-5 w-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div data-market-loading class="hidden items-center justify-center py-12">
        ...
        <span class="market-muted ml-3">{{ __('market.loading') }}</span>
    </div>
    ...
    <div data-market-error class="hidden flex-col items-center justify-center py-12 text-center">
        ...
        <p class="mb-4 text-red-400">{{ __('market.error_loading') }}</p>
    ...
    <div
        data-market-grid
        class="grid grid-cols-1 gap-3 sm:grid-cols-2 {{ $compact ? '2xl:grid-cols-2' : 'xl:grid-cols-3 2xl:grid-cols-4' }}"
    ></div>
</div>

<script>
document.addEventListener('DOMContentLoaded', function () {
    const root = document.getElementById(@json($trackerId));
    ...
    const lastUpdatePrefix = @json(__('market.last_update_prefix'));
    ...
    function getDisplayLabel(item) {
        return {
            ...
            secondary: item.name || item.pair || item.symbol || @json(__('market.title_tracker')),
        };
    }
