...
@section('dashboard_page_title', __('profile.page_title'))
...
@section('sidebar_primary')
    ...
    <a href="{{ localized_route('profile') }}" class="premium-nav-item is-active flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-semibold text-slate-900">
        ...
        <div class="relative z-10">
            ...
            <div class="mt-5 rounded-2xl bg-white/10 px-4 py-3">
                <div class="flex items-center justify-between gap-3">
                    ...
                    <a href="{{ localized_route('transactions.history') }}" class="inline-flex items-center gap-2 rounded-full bg-white/90 px-3 py-2 text-xs font-semibold text-slate-900">
                        {{ __('profile.view_all') }}
                        <i class="fas fa-arrow-right text-[10px]"></i>
                    </a>
                </div>
                <div class="mt-3 h-2.5 rounded-full bg-white/20">
                    <div class="h-2.5 rounded-full bg-white" style="width: {{ $profileCompletion }}%"></div>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="min-w-0">
                <div class="flex min-w-0 flex-col gap-5 sm:flex-row sm:items-center">
                    <div class="profile-avatar-shell flex h-24 w-24 items-center justify-center overflow-hidden rounded-[28px] shadow-lg shadow-emerald-950/20 ring-1 ring-white/20">
                        @if($profilePhotoUrl)
                            <img src="{{ $profilePhotoUrl }}" alt="{{ trim(($user->first_name ?? '') . ' ' . ($user->last_name ?? '')) }}" class="h-full w-full object-cover">
                        @else
                    ...
                    <div class="min-w-0">
                        <p class="text-sm uppercase tracking-[0.22em] text-white/65">{{ __('dashboard.premium_profile') }}</p>
                        <h2 class="premium-page-title mt-3 text-3xl font-semibold tracking-[-0.05em] sm:text-4xl">
                            {{ trim(($user->first_name ?? '') . ' ' . ($user->last_name ?? '')) ?: __('profile.page_title') }}
                        </h2>
                        <p class="mt-2 max-w-2xl text-sm leading-6 text-white/80">{{ $user->email }}</p>
                        <div class="mt-4 flex flex-wrap gap-2">
                            <span class="rounded-full bg-white/12 px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.16em] text-white">
                                {{ __('profile.account_number', ['number' => $user->id]) }}
                            </span>
                            <span class="rounded-full bg-white/12 px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.16em] text-white">
                                {{ $accountStatusLabel }}
                            </span>
    ...
    <div class="grid gap-6 2xl:grid-cols-[minmax(0,1.6fr)_minmax(320px,380px)]">
            ...
            <section class="premium-panel premium-card-hover rounded-[30px] p-5 sm:p-6">
                <div class="flex flex-col gap-3 border-b border-slate-200/70 pb-5 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                        ...
                        <h2 class="mt-2 premium-brand-title text-2xl font-semibold text-slate-950">{{ __('profile.personal_info_title') }}</h2>
                        <p class="mt-2 text-sm leading-6 text-slate-500">{{ __('profile.personal_info_subtitle') }}</p>
                    </div>
                ...
                <div class="profile-info-grid mt-6 grid gap-4">
                    @foreach($profileInfoCards as $card)
            ...
            <section class="premium-panel premium-card-hover rounded-[30px] p-5 sm:p-6">
                <div class="flex flex-col gap-3 border-b border-slate-200/70 pb-5 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                        ...
                        <h2 class="mt-2 premium-brand-title text-2xl font-semibold text-slate-950">{{ __('profile.recent_activity_title') }}</h2>
                        <p class="mt-2 text-sm leading-6 text-slate-500">{{ __('profile.recent_activity_subtitle') }}</p>
                    </div>
                ...
                <div class="mt-6 space-y-3">
                            ...
                            <div class="flex min-w-0 flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                                <div class="flex min-w-0 items-start gap-3">
                                    <span class="mt-0.5 flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl
                                        @if($transaction->type === 'deposit') bg-emerald-50 text-emerald-700
                                        @elseif($transaction->type === 'withdrawal') bg-rose-50 text-rose-700
                                        @else bg-orange-50 text-orange-600 @endif">
                                        <i class="fas fa-@if($transaction->type === 'deposit') arrow-down @elseif($transaction->type === 'withdrawal') arrow-up @else paper-plane @endif"></i>
                                    </span>
                                    <div class="min-w-0">
                                        ...
                                        @if($transaction->recipient_name)
                                            <p class="mt-2 text-sm text-slate-500">{{ $transaction->recipient_name }}</p>
                                        @endif
                                    </div>
                                </div>
                                <div class="min-w-0 sm:text-right">
                                    <p class="profile-info-value text-lg font-semibold {{ $amountColor }}">
                                        {{ $amountPrefix }}{{ \App\Helpers\CurrencyHelper::format($transaction->amount, $currencyCode) }}
                                    </p>
                                    <span class="mt-2 inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-semibold {{ $statusClass }}">
                    ...
                    @empty
                        <div class="rounded-[24px] border border-dashed border-slate-200 bg-slate-50 px-5 py-10 text-center">
                            ...
                            <h3 class="mt-5 text-lg font-semibold text-slate-950">{{ __('profile.no_transactions') }}</h3>
                            <p class="mt-2 text-sm leading-6 text-slate-500">{{ __('profile.no_transactions_message') }}</p>
                        </div>
                    @endforelse
                </div>
            </section>
        ...
        <aside class="space-y-6">
            <section class="premium-panel premium-card-hover rounded-[30px] p-5">
                <p class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">{{ __('profile.banking_info_title') }}</p>
                ...
                <div class="mt-5 space-y-4">
                    <div class="profile-side-card profile-stat-card rounded-[24px] px-4 py-4">
                        <p class="text-xs uppercase tracking-[0.16em] text-slate-400">{{ __('profile.current_balance') }}</p>
                        <p class="profile-balance-value premium-brand-title mt-2 font-semibold text-slate-950">{{ $balanceFormatted }}</p>
                        <p class="mt-2 text-sm text-slate-500">{{ __('profile.available_balance') }}</p>
                    </div>
                    ...
                    <div class="rounded-[24px] bg-slate-50 px-4 py-4 ring-1 ring-slate-200/70">
                        <div class="profile-data-row flex flex-col gap-1 py-3 first:pt-0 last:pb-0 sm:flex-row sm:items-center sm:justify-between">
                            <span class="text-sm text-slate-500">{{ __('profile.account_number_label') }}</span>
                            <span class="profile-info-value text-sm font-semibold text-slate-950 sm:text-right">{{ $user->id }}</span>
                        </div>
                        <div class="profile-data-row flex flex-col gap-1 py-3 first:pt-0 last:pb-0 sm:flex-row sm:items-center sm:justify-between">
                            <span class="text-sm text-slate-500">{{ __('profile.account_status_label') }}</span>
                            <span class="inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-semibold {{ $accountStatusBadgeClass }}">
                                <i class="fas fa-circle text-[8px]"></i>
                                {{ $accountStatusLabel }}
                            </span>
                        </div>
                        <div class="profile-data-row flex flex-col gap-1 py-3 first:pt-0 last:pb-0 sm:flex-row sm:items-center sm:justify-between">
                            <span class="text-sm text-slate-500">{{ __('profile.account_type_label') }}</span>
                            <span class="profile-info-value text-sm font-semibold text-slate-950 sm:text-right">{{ $accountTypeLabel }}</span>
                        </div>
                        <div class="profile-data-row flex flex-col gap-1 py-3 first:pt-0 last:pb-0 sm:flex-row sm:items-center sm:justify-between">
                            <span class="text-sm text-slate-500">{{ __('profile.signup_date_label') }}</span>
                            <span class="profile-info-value text-sm font-semibold text-slate-950 sm:text-right">{{ $user->created_at->format('d/m/Y') }}</span>
                        </div>
                        <div class="flex flex-col gap-1 pt-3 sm:flex-row sm:items-center sm:justify-between">
                            <span class="text-sm text-slate-500">{{ __('profile.default_currency') }}</span>
                            <span class="profile-info-value text-sm font-semibold text-slate-950 sm:text-right">{{ $currencyCode }}</span>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section class="premium-panel premium-card-hover rounded-[30px] p-5">
                <p class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">{{ __('profile.banking_details_title') }}</p>
                <h3 class="mt-2 premium-brand-title text-2xl font-semibold text-slate-950">{{ __('profile.banking_details_subtitle') }}</h3>
                <div class="mt-5 space-y-4">
                    <div class="profile-side-card rounded-[24px] px-4 py-4">
                        <p class="text-xs uppercase tracking-[0.16em] text-slate-400">{{ __('profile.iban') }}</p>
                        <p class="profile-info-value mt-2 text-sm font-semibold leading-6 text-slate-950">{{ $formattedIban }}</p>
                    </div>
                    <div class="profile-side-card rounded-[24px] px-4 py-4">
                        <p class="text-xs uppercase tracking-[0.16em] text-slate-400">{{ __('profile.bic') }}</p>
                        <p class="profile-info-value mt-2 text-sm font-semibold leading-6 text-slate-950">{{ $formattedBic }}</p>
                    </div>
                </div>
            </section>

            <section class="premium-panel premium-card-hover rounded-[30px] p-5">
                <p class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400">{{ __('profile.credit_card_title') }}</p>
                <h3 class="mt-2 premium-brand-title text-2xl font-semibold text-slate-950">{{ __('profile.credit_card_subtitle') }}</h3>

                @if($user->creditCard)
                    <div class="mt-5 space-y-4">
                        <div class="profile-side-card rounded-[24px] px-4 py-4">
                            <p class="text-xs uppercase tracking-[0.16em] text-slate-400">{{ __('profile.card_holder_name') }}</p>
                            <p class="profile-info-value mt-2 text-sm font-semibold leading-6 text-slate-950">{{ $user->creditCard->card_holder_name }}</p>
                        </div>
                        <div class="profile-side-card rounded-[24px] px-4 py-4">
                            <p class="text-xs uppercase tracking-[0.16em] text-slate-400">{{ __('profile.card_number') }}</p>
                            <p class="profile-info-value mt-2 text-sm font-semibold leading-6 text-slate-950">{{ $user->creditCard->masked_card_number }}</p>
                        </div>
                        <div class="grid gap-4 sm:grid-cols-2">
                            <div class="profile-side-card rounded-[24px] px-4 py-4">
                                <p class="text-xs uppercase tracking-[0.16em] text-slate-400">{{ __('profile.card_type') }}</p>
                                <p class="profile-info-value mt-2 text-sm font-semibold leading-6 text-slate-950">{{ $user->creditCard->card_type ?? __('profile.not_specified') }}</p>
                            </div>
                            <div class="profile-side-card rounded-[24px] px-4 py-4">
                                <p class="text-xs uppercase tracking-[0.16em] text-slate-400">{{ __('profile.expiry_date') }}</p>
                                <p class="profile-info-value mt-2 text-sm font-semibold leading-6 text-slate-950">{{ $cardExpiry }}</p>
                            </div>
                        </div>
                    </div>
                @else
                    <div class="mt-5 rounded-[24px] border border-dashed border-slate-200 bg-slate-50 px-4 py-5 text-sm leading-6 text-slate-500">
                        {{ __('profile.no_credit_card') }}
                    </div>
                @endif
            </section>
            ...
            <section class="premium-panel premium-card-hover rounded-[30px] p-5">
                ...
                <div class="mt-5 space-y-4">
                    ...
                    <div class="profile-side-card rounded-[24px] px-4 py-4">
                        ...
                        <div class="profile-data-row flex flex-col gap-1 py-3 first:pt-0 last:pb-0 sm:flex-row sm:items-center sm:justify-between">
                            <span class="text-sm text-slate-500">{{ __('profile.two_factor_label') }}</span>
                            <span class="inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-semibold {{ $user->two_factor_enabled ? 'bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200/80' : 'bg-slate-100 text-slate-700 ring-1 ring-slate-200' }}">
                        ...
                        <div class="flex flex-col gap-2 pt-3 sm:flex-row sm:items-center sm:justify-between">
                            <span class="text-sm text-slate-500">{{ __('profile.configure_2fa') }}</span>
                            <a href="{{ localized_route('twofactor.setup') }}" class="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 shadow-sm transition hover:border-slate-300 hover:bg-slate-50">
                                {{ __('profile.manage') }}
                                <i class="fas fa-arrow-right text-[10px]"></i>
                            </a>
                        </div>
            ...
            <section class="premium-gradient-card rounded-[30px] p-5">
                <p class="text-xs uppercase tracking-[0.18em] text-white/65">{{ __('profile.quick_actions_title') }}</p>
                <h3 class="mt-2 premium-brand-title text-2xl font-semibold">{{ __('profile.quick_actions_subtitle') }}</h3>
                <div class="mt-5 space-y-3">
                    <a href="{{ localized_route('transfer.create') }}" class="inline-flex w-full items-center justify-between gap-3 rounded-[22px] bg-white/10 px-4 py-4 text-sm font-semibold text-white transition hover:bg-white/16">
                        <span class="flex items-center gap-3">
                            <span class="flex h-10 w-10 items-center justify-center rounded-xl bg-white/14">
                                <i class="fas fa-paper-plane"></i>
                            </span>
                            <span>{{ __('profile.new_transfer') }}</span>
                        </span>
                        <i class="fas fa-arrow-right text-[11px]"></i>
                    </a>
                    <a href="{{ localized_route('transactions.history') }}" class="inline-flex w-full items-center justify-between gap-3 rounded-[22px] bg-white/10 px-4 py-4 text-sm font-semibold text-white transition hover:bg-white/16">
                        <span class="flex items-center gap-3">
                            <span class="flex h-10 w-10 items-center justify-center rounded-xl bg-white/14">
                                <i class="fas fa-clock-rotate-left"></i>
                            </span>
                            <span>{{ __('profile.history') }}</span>
                        </span>
                        <i class="fas fa-arrow-right text-[11px]"></i>
                    </a>
                    <a href="{{ localized_route('dashboard') }}" class="inline-flex w-full items-center justify-between gap-3 rounded-[22px] bg-white/10 px-4 py-4 text-sm font-semibold text-white transition hover:bg-white/16">
                        <span class="flex items-center gap-3">
                            <span class="flex h-10 w-10 items-center justify-center rounded-xl bg-white/14">
                                <i class="fas fa-chart-pie"></i>
                            </span>
                            <span>{{ __('profile.back_to_dashboard') }}</span>
                        </span>
                        <i class="fas fa-arrow-right text-[11px]"></i>
                    </a>
                </div>
