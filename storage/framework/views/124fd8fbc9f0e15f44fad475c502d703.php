<?php $__env->startSection('title', 'Créer un utilisateur - Valtrix Bank Admin'); ?>
<?php $__env->startSection('admin_nav_active', 'users-create'); ?>
<?php $__env->startSection('dashboard_page_title', "Création d'utilisateur"); ?>
<?php $__env->startSection('dashboard_page_subtitle', 'Créez un nouveau client ou administrateur avec une fiche complète, claire et professionnelle.'); ?>
<?php $__env->startSection('dashboard_section_label', 'Création de comptes'); ?>
    ...
    <div class="grid gap-6 2xl:grid-cols-[minmax(0,1.6fr)_380px]">
        <section class="admin-surface rounded-[30px] p-5 sm:p-6">
            <div class="border-b border-slate-200/70 pb-5">
                ...
                <p class="mt-2 text-sm leading-6 text-slate-500">Renseignez les informations personnelles, de sécurité, d'adresse et bancaires du compte.</p>
            </div>
            ...
            <form method="POST" action="<?php echo e(localized_route('admin.users.store')); ?>" class="mt-6 space-y-6" enctype="multipart/form-data">
                ...
                <section class="admin-section rounded-[24px] p-5">
                    ...
                    <div class="mt-5 grid gap-4 md:grid-cols-2">
                        <div><label for="first_name" class="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Prénom *</label><input type="text" name="first_name" id="first_name" value="<?php echo e(old('first_name')); ?>" class="admin-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700" placeholder="Jean" required></div>
                        ...
                        <div><label for="phone" class="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Téléphone</label><input type="tel" name="phone" id="phone" value="<?php echo e(old('phone')); ?>" class="admin-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700" placeholder="+33612345678" autocomplete="tel" inputmode="tel"><p class="mt-2 text-sm text-slate-500">Format international uniquement : + indicatif pays + numero.</p></div>
                ...
                <section class="admin-section rounded-[24px] p-5">
                    <h3 class="text-lg font-semibold text-slate-950">Sécurité et authentification</h3>
                    <div class="mt-5 grid gap-4 md:grid-cols-2">
                ...
                <section class="admin-section rounded-[24px] p-5">
                    <h3 class="text-lg font-semibold text-slate-950">Pièce d'identité</h3>
                    <div class="mt-5 grid gap-4 md:grid-cols-2">
                        <div><label for="type_piece" class="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Type</label><select name="type_piece" id="type_piece" class="admin-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700"><option value="">Selectionner</option><option value="CNI" <?php echo e(old('type_piece') == 'CNI' ? 'selected' : ''); ?>>Carte nationale d'identité</option><option value="Passeport" <?php echo e(old('type_piece') == 'Passeport' ? 'selected' : ''); ?>>Passeport</option><option value="Permis" <?php echo e(old('type_piece') == 'Permis' ? 'selected' : ''); ?>>Permis de conduire</option></select></div>
                ...
                <section class="rounded-[24px] bg-blue-50 px-5 py-5 ring-1 ring-blue-200/80">
                    ...
                    <div class="mt-4 max-w-xl"><input type="text" name="activation_code" id="activation_code" value="<?php echo e(old('activation_code')); ?>" class="admin-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700" placeholder="Entrez un code d'activation sécurisé"></div>
                </section>
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:justify-end">
                    ...
                    <button type="submit" class="inline-flex items-center justify-center gap-2 rounded-full bg-blue-700 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-blue-900/20 transition hover:bg-blue-800"><i class="fas fa-user-plus text-xs"></i> Créer l'utilisateur</button>
                </div>
            </form>
        </section>
                ...
                <div class="mt-5 space-y-3">
                    ...
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                        <div class="rounded-[22px] border border-dashed border-slate-300 bg-slate-50 px-4 py-8 text-center">
                            <p class="text-sm font-semibold text-slate-900">Aucun compte récent</p>
                            <p class="mt-2 text-sm text-slate-500">Les derniers comptes créés apparaîtront ici.</p>
                        </div>
                    <?php endif; ?>
                </div>
            </section>
