...
<?php $__env->startSection('title', 'Pilotage des transactions - Valtrix Bank Admin'); ?>
<?php $__env->startSection('admin_nav_active', 'transactions'); ?>
<?php $__env->startSection('dashboard_page_title', 'Gestion des transactions'); ?>
<?php $__env->startSection('dashboard_page_subtitle', 'Supervisez les virements, dépôts, retraits et remboursements depuis une interface premium alignée sur le centre de contrôle admin.'); ?>
<?php $__env->startSection('dashboard_section_label', 'Pilotage des transactions'); ?>

<?php $__env->startSection('dashboard_header_actions'); ?>
    <a href="<?php echo e(localized_route('admin.export.pdf')); ?>" class="inline-flex w-full items-center justify-center gap-2 rounded-full bg-blue-700 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-blue-900/20 transition hover:bg-blue-800 sm:w-auto">
        <i class="fas fa-file-pdf text-xs"></i>
        Export PDF
    </a>
    <a href="<?php echo e(localized_route('admin.deposit')); ?>" class="inline-flex w-full items-center justify-center gap-2 rounded-full border border-slate-200 bg-white px-5 py-3 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-slate-300 hover:bg-slate-50 sm:w-auto">
    ...
    <section class="premium-gradient-card premium-grid-glow relative overflow-hidden rounded-[30px] p-6 sm:p-7">
        <div class="relative z-10 grid gap-3 sm:grid-cols-2 2xl:grid-cols-4">
            <div class="rounded-[24px] bg-white/10 px-4 py-4 backdrop-blur-sm">
                ...
                <p class="premium-kpi-number mt-2 text-2xl font-semibold"><?php echo e($filteredTransactionsCount); ?></p>
                <p class="mt-2 text-xs text-white/72"><?php echo e($transactionsTodayCount); ?> aujourd'hui dans la vue courante.</p>
            </div>
            <div class="rounded-[24px] bg-white/10 px-4 py-4 backdrop-blur-sm">
                <p class="text-xs uppercase tracking-[0.18em] text-white/60">Volume filtre</p>
                <p class="admin-amount premium-kpi-number mt-2 font-semibold"><?php echo e($filteredTransactionsVolumeFormatted); ?></p>
            ...
            <div class="rounded-[24px] bg-white/10 px-4 py-4 backdrop-blur-sm">
                ...
                <p class="premium-kpi-number mt-2 text-2xl font-semibold"><?php echo e($filteredReviewCount); ?></p>
                <p class="mt-2 text-xs text-white/72"><?php echo e($filteredRefundedCount); ?> déjà remboursées.</p>
            </div>
            <div class="rounded-[24px] bg-white/10 px-4 py-4 backdrop-blur-sm">
                <p class="text-xs uppercase tracking-[0.18em] text-white/60">Taux de succès</p>
                <p class="premium-kpi-number mt-2 text-2xl font-semibold"><?php echo e($filteredSuccessRate); ?>%</p>
                <p class="mt-2 text-xs text-white/72"><?php echo e($filteredSuccessCount); ?> opération(s) réussie(s).</p>
            </div>
        </div>
    </section>
    ...
    <div class="grid gap-6 2xl:grid-cols-[minmax(0,1.65fr)_360px]">
        <section class="space-y-6">
            <section class="admin-surface rounded-[30px] p-5 sm:p-6">
                <div class="flex flex-col gap-4 border-b border-slate-200/70 pb-5 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                        ...
                        <p class="mt-2 text-sm leading-6 text-slate-500">
                            <?php echo e($activeFiltersCount); ?> filtre(s) actif(s). Ajustez la recherche pour cibler un utilisateur, un statut ou une période.
                        </p>
                    </div>
                ...
                <form method="GET" action="<?php echo e(localized_route('admin.transactions')); ?>" class="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                    <div class="xl:col-span-2">
                        <label for="search" class="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Recherche</label>
                        <input type="text" name="search" id="search" value="<?php echo e(request('search')); ?>" placeholder="ID, client, email ou bénéficiaire..." class="admin-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <select name="status" id="status" class="admin-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700">
                            ...
                            <option value="on_hold" <?php echo e(request('status') === 'on_hold' ? 'selected' : ''); ?>>En hold</option>
                            <option value="refunded" <?php echo e(request('status') === 'refunded' ? 'selected' : ''); ?>>Rembourse</option>
                        </select>
                    </div>
                    <div class="xl:col-span-2">
                    ...
                    <div>
                        ...
                        <input type="date" name="date_from" id="date_from" value="<?php echo e(request('date_from')); ?>" class="admin-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700">
                    </div>
                    <div>
                        <label for="date_to" class="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Date de fin</label>
                        <input type="date" name="date_to" id="date_to" value="<?php echo e(request('date_to')); ?>" class="admin-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700">
                    </div>
                    <div class="xl:col-span-4 flex flex-col gap-3 pt-2 sm:flex-row sm:justify-end">
                        <a href="<?php echo e(localized_route('admin.transactions')); ?>" class="inline-flex items-center justify-center gap-2 rounded-full border border-slate-200 bg-white px-5 py-3 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50">
            ...
            <section class="admin-surface rounded-[30px] p-5 sm:p-6">
                <div class="flex flex-col gap-4 border-b border-slate-200/70 pb-5 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                        ...
                        <p class="mt-2 text-sm leading-6 text-slate-500">
                            Affichage de <?php echo e($transactions->count()); ?> ligne(s) sur <?php echo e($transactions->total()); ?> resultat(s).
                        </p>
                    </div>
                    <div class="flex flex-wrap gap-2">
                        <span class="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-4 py-2 text-xs font-semibold text-emerald-700 ring-1 ring-emerald-200/80">
                            ...
                            <?php echo e($filteredSuccessCount); ?> succès
                        </span>
                        <span class="inline-flex items-center gap-2 rounded-full bg-amber-50 px-4 py-2 text-xs font-semibold text-amber-700 ring-1 ring-amber-200/80">
                            ...
                            <?php echo e($filteredReviewCount); ?> à revoir
                        </span>
                    </div>
                </div>
                                ...
                                <div class="flex items-start justify-between gap-3">
                                    <div class="min-w-0 flex-1">
                                        <div class="flex flex-wrap items-center gap-2">
                                            <p class="text-sm font-semibold text-slate-900">#<?php echo e($transaction->id); ?></p>
                                            <span class="inline-flex items-center gap-2 rounded-full px-3 py-1 text-[11px] font-semibold <?php echo e($typeConfig['class']); ?>">
                                                <i class="fas <?php echo e($typeConfig['icon']); ?> text-[10px]"></i>
                                                <?php echo e($typeConfig['label']); ?>

                                            </span>
                                        </div>
                                        <p class="admin-mobile-value mt-2 text-sm text-slate-500"><?php echo e($transaction->reason ?: 'Aucun motif renseigné'); ?></p>
                                    </div>
                                    <div class="shrink-0 text-right">
                                        <p class="text-base font-semibold text-slate-900"><?php echo e(\App\Helpers\CurrencyHelper::format($transaction->amount, $transaction->user->default_currency ?? 'EUR')); ?></p>
                                        <p class="mt-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-400"><?php echo e($transaction->user->default_currency ?? 'EUR'); ?></p>
                                    </div>
                                </div>
                                ...
                                <div class="rounded-[20px] bg-slate-50 px-4 py-4 ring-1 ring-slate-200/70">
                                    <div class="admin-mobile-row text-sm text-slate-600">
                                        ...
                                        <div class="admin-mobile-value">
                                            <p class="font-semibold text-slate-900"><?php echo e($transaction->user?->first_name); ?> <?php echo e($transaction->user?->last_name); ?></p>
                                            <p class="mt-1 text-sm text-slate-500"><?php echo e($transaction->user?->email); ?></p>
                                        </div>
                                    </div>
                                    <div class="admin-mobile-row text-sm text-slate-600">
                                        ...
                                        <div class="admin-mobile-value">
                                            <?php if($transaction->recipient_name): ?>
                                                <p class="font-semibold text-slate-900"><?php echo e($transaction->recipient_name); ?></p>
                                                <p class="mt-1 text-sm text-slate-500"><?php echo e($transaction->bank_name ?: 'Banque non renseignée'); ?></p>
                                            <?php else: ?>
                                    ...
                                    <div class="admin-mobile-row text-sm text-slate-600">
                                        <span>Statut</span>
                                        <span class="inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-semibold <?php echo e($statusConfig['class']); ?>">
                                            <i class="fas <?php echo e($statusConfig['icon']); ?> text-[10px]"></i>
                                    ...
                                    <div class="admin-mobile-row text-sm text-slate-600">
                                        <span>Date</span>
                                        <div class="admin-mobile-value">
                                            <p class="font-semibold text-slate-900"><?php echo e($transaction->created_at->format('d/m/Y H:i')); ?></p>
                                            <p class="mt-1 text-xs uppercase tracking-[0.16em] text-slate-400"><?php echo e($transaction->created_at->diffForHumans()); ?></p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="flex flex-col gap-2">
                                    <?php if($transaction->status === 'success' && $transaction->type === 'transfer'): ?>
                                        <button
                                            ...
                                            onclick="openRefundModal(<?php echo e($transaction->id); ?>, <?php echo \Illuminate\Support\Js::from(($transaction->user?->first_name ?? '') . ' ' . ($transaction->user?->last_name ?? ''))->toHtml() ?>, <?php echo e((float) $transaction->amount); ?>, <?php echo \Illuminate\Support\Js::from($transaction->user->default_currency ?? 'EUR')->toHtml() ?>)"
                                        >
                                    ...
                                    <?php elseif($transaction->status === 'refunded'): ?>
                                        <span class="inline-flex w-full items-center justify-center gap-2 rounded-full bg-violet-50 px-4 py-3 text-xs font-semibold text-violet-700 ring-1 ring-violet-200/80">
                                            <i class="fas fa-check text-[11px]"></i>
                                            <?php echo e($transaction->refunded_at ? 'Le ' . $transaction->refunded_at->format('d/m/Y') : 'Traitee'); ?>
                            ...
                            <tbody class="divide-y divide-slate-200 bg-white">
                                <?php $__empty_1 = true; $__currentLoopData = $transactions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $transaction): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                    ...
                                    <tr class="admin-row">
                                        <td class="px-4 py-4">
                                            <div class="min-w-0">
                                                <div class="flex items-center gap-2">
                                                    <p class="font-semibold text-slate-900">#<?php echo e($transaction->id); ?></p>
                                                    <span class="inline-flex items-center gap-2 rounded-full px-3 py-1 text-[11px] font-semibold <?php echo e($typeConfig['class']); ?>">
                                                        <i class="fas <?php echo e($typeConfig['icon']); ?> text-[10px]"></i>
                                                        <?php echo e($typeConfig['label']); ?>
                                                ...
                                                <p class="mt-2 text-sm text-slate-500"><?php echo e($transaction->reason ?: 'Aucun motif renseigné'); ?></p>
                                            </div>
                                        </td>
                                        <td class="px-4 py-4">
                                            <p class="font-semibold text-slate-900"><?php echo e($transaction->user?->first_name); ?> <?php echo e($transaction->user?->last_name); ?></p>
                                        ...
                                        <td class="px-4 py-4">
                                            <p class="font-semibold text-slate-900"><?php echo e(\App\Helpers\CurrencyHelper::format($transaction->amount, $transaction->user->default_currency ?? 'EUR')); ?></p>
                                            <p class="mt-1 text-xs uppercase tracking-[0.16em] text-slate-400"><?php echo e($transaction->user->default_currency ?? 'EUR'); ?></p>
                                        </td>
                                        <td class="px-4 py-4">
                                            <?php if($transaction->recipient_name): ?>
                                                <p class="font-semibold text-slate-900"><?php echo e($transaction->recipient_name); ?></p>
                ...
                <?php if($transactions->hasPages()): ?>
                    <div class="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <p class="text-sm text-slate-500">Affichage de <?php echo e($transactions->firstItem()); ?> a <?php echo e($transactions->lastItem()); ?> sur <?php echo e($transactions->total()); ?> transactions</p>
                        <div class="overflow-x-auto pb-1"><?php echo e($transactions->links('vendor.pagination.tailwind')); ?></div>
                    </div>
                <?php endif; ?>
            </section>
        </section>
                    ...
                    <?php $__empty_1 = true; $__currentLoopData = $recentTransactions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $transaction): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <div class="rounded-[22px] bg-slate-50 px-4 py-4 ring-1 ring-slate-200/70">
                            <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                <div class="min-w-0">
                                    <p class="text-sm font-semibold text-slate-900">#<?php echo e($transaction->id); ?> - <?php echo e($transaction->user?->first_name); ?> <?php echo e($transaction->user?->last_name); ?></p>
                                    <p class="mt-1 text-xs uppercase tracking-[0.16em] text-slate-400"><?php echo e(ucfirst(str_replace('_', ' ', $transaction->type))); ?> | <?php echo e(ucfirst(str_replace('_', ' ', $transaction->status))); ?></p>
                                    <p class="mt-2 text-sm text-slate-500"><?php echo e($transaction->created_at->format('d/m/Y H:i')); ?></p>
                                </div>
                                <span class="admin-mobile-value text-sm font-semibold text-slate-900 sm:text-right"><?php echo e(\App\Helpers\CurrencyHelper::format($transaction->amount, $transaction->user->default_currency ?? 'EUR')); ?></span>
                            </div>
                        </div>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
...
<?php $__env->startPush('premium_dashboard_scripts'); ?>
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const refundBaseUrl = <?php echo json_encode(url(app()->getLocale() . '/admin/transactions'), 15, 512) ?>;
...
<?php echo $__env->make('layouts.admin-premium', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\cerveau\resources\views/admin/transactions.blade.php ENDPATH**/ ?>