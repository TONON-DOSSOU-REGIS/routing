...
<?php $__env->startSection('title', __('notifications.page_title')); ?>
<?php $__env->startSection('dashboard_theme', 'client'); ?>
<?php $__env->startSection('dashboard_page_title', __('notifications.title')); ?>
<?php $__env->startSection('dashboard_page_subtitle', __('notifications.subtitle')); ?>
<?php $__env->startSection('dashboard_section_label', __('dashboard.notifications')); ?>
...
<?php $__env->startSection('sidebar_secondary'); ?>
    <a href="<?php echo e(localized_route('notifications.index')); ?>" class="premium-nav-item is-active flex items-center justify-between gap-3 rounded-2xl px-4 py-3 text-sm font-semibold text-slate-900">
        ...
        <span id="sidebar-unread-count" class="rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-bold text-emerald-700"><?php echo e($unreadNotificationsCount); ?></span>
    </a>
...
<?php $__env->startSection('sidebar_footer'); ?>
    <div class="premium-gradient-card premium-grid-glow relative overflow-hidden rounded-[26px] p-5">
        <div class="relative z-10">
            ...
            <p class="mt-2 text-sm leading-6 text-white/78"><?php echo e(__('dashboard.concierge_description')); ?></p>
...
<?php $__env->startSection('dashboard_content'); ?>
    <section class="premium-gradient-card premium-grid-glow relative overflow-hidden rounded-[30px] p-6 sm:p-7">
        <div class="relative z-10 flex flex-col gap-6 2xl:flex-row 2xl:items-end 2xl:justify-between">
            <div class="max-w-3xl">
                <p class="text-sm uppercase tracking-[0.22em] text-white/65"><?php echo e(__('dashboard.notification_center')); ?></p>
                <h2 class="mt-4 premium-page-title text-3xl font-semibold tracking-[-0.05em] sm:text-4xl"><?php echo e(__('notifications.title')); ?></h2>
                <p class="mt-3 text-sm leading-6 text-white/78 sm:text-base"><?php echo e(__('notifications.subtitle')); ?></p>
            </div>
            <div class="grid gap-3 sm:grid-cols-2 2xl:grid-cols-4 2xl:min-w-[720px]">
                <div class="rounded-[24px] bg-white/10 px-4 py-4 backdrop-blur-sm"><p class="text-xs uppercase tracking-[0.18em] text-white/60"><?php echo e(__('notifications.summary_total')); ?></p><p id="stat-total" class="notification-stat premium-kpi-number mt-2 font-semibold"><?php echo e($totalNotificationsCount); ?></p></div>
                <div class="rounded-[24px] bg-white/10 px-4 py-4 backdrop-blur-sm"><p class="text-xs uppercase tracking-[0.18em] text-white/60"><?php echo e(__('notifications.summary_unread')); ?></p><p id="stat-unread" class="notification-stat premium-kpi-number mt-2 font-semibold"><?php echo e($unreadNotificationsCount); ?></p></div>
                <div class="rounded-[24px] bg-white/10 px-4 py-4 backdrop-blur-sm"><p class="text-xs uppercase tracking-[0.18em] text-white/60"><?php echo e(__('notifications.summary_read')); ?></p><p id="stat-read" class="notification-stat premium-kpi-number mt-2 font-semibold"><?php echo e($readNotificationsCount); ?></p></div>
                <div class="rounded-[24px] bg-white/10 px-4 py-4 backdrop-blur-sm"><p class="text-xs uppercase tracking-[0.18em] text-white/60"><?php echo e(__('notifications.summary_last_24h')); ?></p><p id="stat-last24" class="notification-stat premium-kpi-number mt-2 font-semibold"><?php echo e($notificationsLast24HoursCount); ?></p></div>
            </div>
        </div>
    </section>
    ...
    <div class="grid gap-6 2xl:grid-cols-[minmax(0,1.6fr)_minmax(320px,380px)]">
        <section class="space-y-6">
            <section class="premium-panel premium-card-hover rounded-[30px] p-5 sm:p-6">
                <div class="flex flex-col gap-3 border-b border-slate-200/70 pb-5 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                        ...
                        <h2 class="mt-2 premium-brand-title text-2xl font-semibold text-slate-950"><?php echo e(__('notifications.filters_title')); ?></h2>
                        <p class="mt-2 text-sm leading-6 text-slate-500"><?php echo e(__('notifications.filters_subtitle')); ?></p>
                    </div>
                    <span class="premium-soft-chip rounded-full px-3 py-1 text-xs font-semibold"><span id="active-filters-count">0</span> <?php echo e(__('notifications.active_filters')); ?></span>
                </div>
                <div class="mt-6 grid gap-4 md:grid-cols-3">
                    <div><label for="type-filter" class="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-slate-400"><?php echo e(__('notifications.filter_type')); ?></label><select id="type-filter" class="notification-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700"><option value=""><?php echo e(__('notifications.all_types')); ?></option><option value="transaction"><?php echo e(__('notifications.type_transaction')); ?></option><option value="message"><?php echo e(__('notifications.type_message')); ?></option><option value="account"><?php echo e(__('notifications.type_account')); ?></option><option value="alert"><?php echo e(__('notifications.type_alert')); ?></option><option value="system"><?php echo e(__('notifications.type_system')); ?></option></select></div>
                    <div><label for="status-filter" class="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-slate-400"><?php echo e(__('notifications.filter_status')); ?></label><select id="status-filter" class="notification-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700"><option value=""><?php echo e(__('notifications.status_all')); ?></option><option value="unread"><?php echo e(__('notifications.status_unread')); ?></option><option value="read"><?php echo e(__('notifications.status_read')); ?></option></select></div>
                    <div class="flex items-end"><button id="apply-filters" class="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-orange-500 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-orange-900/20 transition hover:bg-orange-600"><i class="fas fa-filter text-xs"></i><?php echo e(__('notifications.filter_apply')); ?></button></div>
                </div>
            </section>
            ...
            <section class="notification-card premium-panel premium-card-hover overflow-hidden rounded-[30px]">
                <div class="flex flex-col gap-4 border-b border-slate-200/70 px-5 py-5 sm:flex-row sm:items-start sm:justify-between sm:px-6">
                    <div><p class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400"><?php echo e(__('dashboard.real_time')); ?></p><h2 class="mt-2 premium-brand-title text-2xl font-semibold text-slate-950"><?php echo e(__('notifications.feed_title')); ?></h2><p class="mt-2 text-sm leading-6 text-slate-500"><?php echo e(__('notifications.feed_subtitle')); ?></p></div>
                    <div class="flex flex-wrap gap-3">
                        <button id="mark-all-read-btn" class="inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-4 py-2.5 text-sm font-semibold text-emerald-700 transition hover:bg-emerald-100"><i class="fas fa-check-double text-xs"></i><?php echo e(__('notifications.mark_all_read')); ?></button>
                        <button id="delete-read-btn" class="inline-flex items-center gap-2 rounded-full border border-rose-200 bg-rose-50 px-4 py-2.5 text-sm font-semibold text-rose-700 transition hover:bg-rose-100"><i class="fas fa-trash text-xs"></i><?php echo e(__('notifications.delete_read')); ?></button>
                    </div>
                </div>
                <div id="notifications-container" class="notification-scroll max-h-[960px] overflow-y-auto"><div class="px-5 py-12 text-center text-slate-500 sm:px-6"><div class="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-slate-100 text-slate-400 ring-1 ring-slate-200"><i class="fas fa-spinner fa-spin text-xl"></i></div><p class="mt-4 text-sm font-medium"><?php echo e(__('notifications.loading')); ?></p></div></div>
        ...
        <aside class="space-y-6">
            <section class="notification-card premium-panel premium-card-hover rounded-[30px] p-5">
                <p class="text-xs font-semibold uppercase tracking-[0.2em] text-slate-400"><?php echo e(__('notifications.archive_title')); ?></p>
                <h3 class="mt-2 premium-brand-title text-2xl font-semibold text-slate-950"><?php echo e(__('dashboard.client_priorities')); ?></h3>
                <p class="mt-2 text-sm leading-6 text-slate-500"><?php echo e(__('notifications.archive_subtitle')); ?></p>
                <div class="mt-5 space-y-4">
                    <div class="rounded-[24px] bg-slate-50 px-4 py-4 ring-1 ring-slate-200/70">
                        <div class="flex items-center justify-between gap-3 py-3"><span class="text-sm text-slate-500"><?php echo e(__('notifications.summary_total')); ?></span><span id="archive-total" class="text-sm font-semibold text-slate-950"><?php echo e($totalNotificationsCount); ?></span></div>
                        <div class="flex items-center justify-between gap-3 py-3"><span class="text-sm text-slate-500"><?php echo e(__('notifications.summary_unread')); ?></span><span id="archive-unread" class="text-sm font-semibold text-slate-950"><?php echo e($unreadNotificationsCount); ?></span></div>
                        <div class="flex items-center justify-between gap-3 py-3"><span class="text-sm text-slate-500"><?php echo e(__('notifications.summary_read')); ?></span><span id="archive-read" class="text-sm font-semibold text-slate-950"><?php echo e($readNotificationsCount); ?></span></div>
                        <div class="flex items-center justify-between gap-3 py-3"><span class="text-sm text-slate-500"><?php echo e(__('notifications.summary_last_24h')); ?></span><span id="archive-last24" class="text-sm font-semibold text-slate-950"><?php echo e($notificationsLast24HoursCount); ?></span></div>
                        <div class="flex items-center justify-between gap-3 py-3"><span class="text-sm text-slate-500"><?php echo e(__('notifications.active_filters')); ?></span><span id="archive-active-filters" class="text-sm font-semibold text-slate-950">0</span></div>
                    </div>
                </div>
            </section>
            ...
            <section class="premium-gradient-card premium-grid-glow relative overflow-hidden rounded-[30px] p-5">
                <div class="relative z-10">
                    <p class="text-xs uppercase tracking-[0.18em] text-white/65"><?php echo e(__('dashboard.quick_pilot')); ?></p>
                    <h3 class="mt-2 premium-brand-title text-2xl font-semibold"><?php echo e(__('notifications.detail_title')); ?></h3>
                    <p class="mt-2 text-sm leading-6 text-white/78"><?php echo e(__('notifications.detail_empty')); ?></p>
                    <div class="mt-5 space-y-3">
                        <a href="<?php echo e(localized_route('dashboard')); ?>" class="inline-flex w-full items-center justify-between gap-3 rounded-[22px] bg-white/10 px-4 py-4 text-sm font-semibold text-white transition hover:bg-white/16"><span class="flex items-center gap-3"><span class="flex h-10 w-10 items-center justify-center rounded-xl bg-white/14"><i class="fas fa-chart-pie"></i></span><span><?php echo e(__('dashboard.dashboard_title')); ?></span></span><i class="fas fa-arrow-right text-[11px]"></i></a>
                        <a href="<?php echo e(localized_route('transactions.history')); ?>" class="inline-flex w-full items-center justify-between gap-3 rounded-[22px] bg-white/10 px-4 py-4 text-sm font-semibold text-white transition hover:bg-white/16"><span class="flex items-center gap-3"><span class="flex h-10 w-10 items-center justify-center rounded-xl bg-white/14"><i class="fas fa-clock-rotate-left"></i></span><span><?php echo e(__('dashboard.history')); ?></span></span><i class="fas fa-arrow-right text-[11px]"></i></a>
                        <a href="<?php echo e(localized_route('profile')); ?>" class="inline-flex w-full items-center justify-between gap-3 rounded-[22px] bg-white/10 px-4 py-4 text-sm font-semibold text-white transition hover:bg-white/16"><span class="flex items-center gap-3"><span class="flex h-10 w-10 items-center justify-center rounded-xl bg-white/14"><i class="fas fa-user-shield"></i></span><span><?php echo e(__('dashboard.profile')); ?></span></span><i class="fas fa-arrow-right text-[11px]"></i></a>
                    </div>
                </div>
            </section>
        </aside>
        ...
        <div class="flex min-h-full items-center justify-center">
            <div class="notification-modal-panel w-full max-w-3xl rounded-[32px] p-5 sm:p-6">
                <div class="flex items-start justify-between gap-4">
                    <div class="min-w-0">
                        <p class="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400"><?php echo e(__('notifications.detail_title')); ?></p>
                        <h3 id="modal-title" class="mt-2 text-2xl font-semibold text-slate-950"><?php echo e(__('notifications.detail_title')); ?></h3>
                    </div>
                ...
                <div class="mt-6 flex flex-wrap items-center gap-3">
                    <span id="modal-status" class="notification-chip bg-slate-100 text-slate-600 ring-1 ring-slate-200"><i class="fas fa-circle text-[8px]"></i><?php echo e(__('notifications.status_read')); ?></span>
                    <span id="modal-date" class="text-sm text-slate-500"><?php echo e(__('notifications.detail_empty')); ?></span>
                </div>
                <div id="modal-content" class="mt-6 whitespace-pre-line text-sm leading-7 text-slate-600"><?php echo e(__('notifications.detail_empty')); ?></div>
            </div>
        </div>
    </div>
...
<?php $__env->startPush('premium_dashboard_scripts'); ?>
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            let currentPage = 1;
            let currentFilters = {};
            const i18n = <?php echo json_encode($i18n, 15, 512) ?>;
            ...
            setActiveFilters({});
            syncUnread(<?php echo e($unreadNotificationsCount); ?>);
            loadNotifications();
        });
    </script>
<?php $__env->stopPush(); ?>
