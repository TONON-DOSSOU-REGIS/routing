...
<?php $__env->startSection('title', 'Modifier un utilisateur - Valtrix Bank Admin'); ?>
<?php $__env->startSection('admin_nav_active', 'users'); ?>
<?php $__env->startSection('dashboard_page_title', 'Édition utilisateur'); ?>
<?php $__env->startSection('dashboard_page_subtitle', 'Une fiche client premium, plus claire et plus professionnelle, alignee sur le reste de l admin.'); ?>
<?php $__env->startSection('dashboard_section_label', 'Gestion des utilisateurs'); ?>

<?php $__env->startSection('dashboard_header_actions'); ?>
    <a href="<?php echo e(localized_route('admin.users')); ?>" class="inline-flex items-center gap-2 rounded-full bg-blue-700 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-blue-900/20 transition hover:bg-blue-800"><i class="fas fa-users text-xs"></i> Retour à la liste</a>
    <a href="<?php echo e(localized_route('admin.users.create')); ?>" class="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-5 py-3 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-slate-300 hover:bg-slate-50"><i class="fas fa-user-plus text-xs"></i> Nouvel utilisateur</a>
    ...
    <section class="premium-gradient-card premium-grid-glow relative overflow-hidden rounded-[30px] p-6 sm:p-7">
        <div class="relative z-10 flex flex-col gap-6 2xl:flex-row 2xl:items-end 2xl:justify-between">
            <div class="flex items-start gap-4">
                <div class="flex h-[88px] w-[88px] items-center justify-center overflow-hidden rounded-[28px] bg-white/15 text-2xl font-semibold text-white"><?php if($photoUrl): ?><img src="<?php echo e($photoUrl); ?>" alt="Photo de <?php echo e($user->name); ?>" class="h-full w-full object-cover"><?php else: ?><?php echo e($initials); ?><?php endif; ?></div>
                <div class="max-w-3xl">
                    <p class="text-sm uppercase tracking-[0.22em] text-white/65">Profil client</p>
                    <h2 class="mt-3 premium-page-title text-3xl font-semibold tracking-[-0.05em] sm:text-4xl"><?php echo e($user->name); ?></h2>
                    ...
                    <div class="mt-4 flex flex-wrap gap-2"><span class="admin-chip bg-white/14 text-white">ID #<?php echo e($user->id); ?></span><span class="admin-chip <?php echo e($statusMeta['class']); ?>"><i class="fas <?php echo e($statusMeta['icon']); ?>"></i><?php echo e($statusMeta['label']); ?></span><span class="admin-chip <?php echo e($roleMeta['class']); ?>"><i class="fas <?php echo e($roleMeta['icon']); ?>"></i><?php echo e($roleMeta['label']); ?></span></div>
                </div>
            </div>
            <div class="grid gap-3 sm:grid-cols-2 2xl:min-w-[560px]">
                <div class="rounded-[24px] bg-white/10 px-4 py-4 backdrop-blur-sm"><p class="text-xs uppercase tracking-[0.18em] text-white/60">Solde actuel</p><p class="premium-kpi-number mt-2 text-2xl font-semibold"><?php echo e($balanceFormatted); ?></p></div>
                <div class="rounded-[24px] bg-white/10 px-4 py-4 backdrop-blur-sm"><p class="text-xs uppercase tracking-[0.18em] text-white/60">Opérations</p><p class="premium-kpi-number mt-2 text-2xl font-semibold"><?php echo e($user->transactions_count ?? 0); ?></p></div>
                <div class="rounded-[24px] bg-white/10 px-4 py-4 backdrop-blur-sm"><p class="text-xs uppercase tracking-[0.18em] text-white/60">En attente</p><p class="premium-kpi-number mt-2 text-2xl font-semibold"><?php echo e($user->pending_transactions_count ?? 0); ?></p></div>
                <div class="rounded-[24px] bg-white/10 px-4 py-4 backdrop-blur-sm"><p class="text-xs uppercase tracking-[0.18em] text-white/60">Membre depuis</p><p class="mt-2 text-lg font-semibold"><?php echo e(optional($user->created_at)->format('d/m/Y') ?: 'N/A'); ?></p></div>
            </div>
        </div>
    </section>

    <?php if(session('status')): ?><div class="rounded-[26px] border border-emerald-200 bg-emerald-50 px-5 py-4 text-sm font-medium text-emerald-800"><?php echo e(session('status')); ?></div><?php endif; ?>
    <?php if($errors->any()): ?><div class="rounded-[26px] border border-rose-200 bg-rose-50 px-5 py-4"><p class="text-sm font-semibold text-rose-800">Erreurs de validation</p><ul class="mt-2 list-disc space-y-1 pl-5 text-sm text-rose-700"><?php $__currentLoopData = $errors->all(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $error): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><li><?php echo e($error); ?></li><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?></ul></div><?php endif; ?>

    <form method="POST" action="<?php echo e(localized_route('admin.users.update', $user)); ?>" class="space-y-6" enctype="multipart/form-data">
        <?php echo csrf_field(); ?>
        ...
        <div class="grid gap-6 2xl:grid-cols-[minmax(0,1.55fr)_380px]">
            <section class="admin-surface rounded-[30px] p-5 sm:p-6">
                ...
                <div class="mt-6 space-y-6">
                    <section class="admin-section rounded-[24px] p-5"><h4 class="text-lg font-semibold text-slate-950">Informations personnelles</h4><div class="mt-5 grid gap-4 md:grid-cols-2"><div><label for="first_name" class="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Prénom *</label><input type="text" name="first_name" id="first_name" value="<?php echo e(old('first_name', $user->first_name)); ?>" class="admin-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700" required></div><div><label for="last_name" class="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Nom *</label><input type="text" name="last_name" id="last_name" value="<?php echo e(old('last_name', $user->last_name)); ?>" class="admin-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700" required></div><div><label for="email" class="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Email *</label><input type="email" name="email" id="email" value="<?php echo e(old('email', $user->email)); ?>" class="admin-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700" required></div><div><label for="phone" class="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Téléphone</label><input type="tel" name="phone" id="phone" value="<?php echo e(old('phone', $user->phone)); ?>" class="admin-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700"></div><div><label for="date_naissance" class="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Date de naissance</label><input type="date" name="date_naissance" id="date_naissance" value="<?php echo e(old('date_naissance', optional($user->date_of_birth)->format('Y-m-d'))); ?>" class="admin-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700"></div><div><label for="role" class="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Rôle *</label><select name="role" id="role" class="admin-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700" required><option value="user" <?php if($role === 'user'): echo 'selected'; endif; ?>>Client</option><option value="admin" <?php if($role === 'admin'): echo 'selected'; endif; ?>>Administrateur</option></select></div><div class="md:col-span-2"><label for="profile_photo" class="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Photo du client</label><input type="file" name="profile_photo" id="profile_photo" accept="image/png,image/jpeg,image/webp" class="admin-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700"><p class="mt-2 text-sm text-slate-500">Formats acceptes : JPG, PNG, WebP (2 Mo max).</p></div></div></section>
                    <section class="admin-section rounded-[24px] p-5"><h4 class="text-lg font-semibold text-slate-950">Adresse et verification</h4><div class="mt-5 grid gap-4 md:grid-cols-2"><div class="md:col-span-2"><label for="adresse" class="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Adresse</label><input type="text" name="adresse" id="adresse" value="<?php echo e(old('adresse', $user->address)); ?>" class="admin-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700"></div><div><label for="ville" class="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Ville</label><input type="text" name="ville" id="ville" value="<?php echo e(old('ville', $user->city)); ?>" class="admin-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700"></div><div><label for="pays" class="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Pays</label><select name="pays" id="pays" class="admin-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700"><option value="">Selectionner un pays</option><?php $__currentLoopData = $countries; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $country): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><option value="<?php echo e($country['name']); ?>" <?php if(old('pays', $user->country) === $country['name']): echo 'selected'; endif; ?>>(<?php echo e($country['code']); ?>) <?php echo e($country['name']); ?></option><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?></select></div><div><label for="type_piece" class="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Type de pièce</label><select name="type_piece" id="type_piece" class="admin-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700"><option value="">Selectionner</option><option value="CNI" <?php if($idType === 'CNI'): echo 'selected'; endif; ?>>Carte nationale d identite</option><option value="Passport" <?php if($idType === 'Passport'): echo 'selected'; endif; ?>>Passeport</option><option value="Permis" <?php if($idType === 'Permis'): echo 'selected'; endif; ?>>Permis de conduire</option></select></div><div><label for="numero_piece" class="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Numero de piece</label><input type="text" name="numero_piece" id="numero_piece" value="<?php echo e(old('numero_piece', $user->id_number)); ?>" class="admin-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700"></div></div></section>
                    <section class="admin-section rounded-[24px] p-5"><h4 class="text-lg font-semibold text-slate-950">Banque et carte virtuelle</h4><div class="mt-5 grid gap-4 md:grid-cols-2"><div><label for="iban" class="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">IBAN</label><input type="text" name="iban" id="iban" value="<?php echo e(old('iban', $user->iban)); ?>" class="admin-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700"></div><div><label for="bic" class="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">BIC</label><input type="text" name="bic" id="bic" value="<?php echo e(old('bic', $user->bic)); ?>" class="admin-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700"></div><div class="md:col-span-2"><label for="activation_code" class="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Code d activation</label><input type="text" name="activation_code" id="activation_code" value="<?php echo e(old('activation_code', $user->activation_code)); ?>" class="admin-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700"></div><div><label for="card_holder_name" class="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Titulaire</label><input type="text" name="card_holder_name" id="card_holder_name" value="<?php echo e(old('card_holder_name', optional($creditCard)->card_holder_name)); ?>" class="admin-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700"></div><div><label for="card_number" class="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Numero de carte</label><input type="text" name="card_number" id="card_number" value="<?php echo e(old('card_number', optional($creditCard)->card_number)); ?>" class="admin-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700"></div><div><label for="card_type" class="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Type</label><input type="text" name="card_type" id="card_type" value="<?php echo e(old('card_type', optional($creditCard)->card_type)); ?>" class="admin-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700"></div><div><label for="expiry_date" class="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Expiration</label><input type="date" name="expiry_date" id="expiry_date" value="<?php echo e(old('expiry_date', optional(optional($creditCard)->expiry_date)->format('Y-m-d'))); ?>" class="admin-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700"></div><div class="md:col-span-2"><label class="flex items-center justify-between rounded-[22px] bg-white px-4 py-4 ring-1 ring-slate-200/80"><span><span class="block text-sm font-semibold text-slate-900">Carte visible pour le client</span><span class="mt-1 block text-sm text-slate-500">Autoriser l'affichage dans l'espace client.</span></span><span class="flex items-center gap-3"><input type="checkbox" name="card_visible_to_user" id="card_visible_to_user" value="1" class="peer sr-only" <?php if((bool) old('card_visible_to_user', (bool) optional($creditCard)->is_visible_to_user)): echo 'checked'; endif; ?>><span class="relative inline-flex h-6 w-11 items-center rounded-full bg-slate-300 transition peer-checked:bg-blue-700 after:h-4 after:w-4 after:translate-x-1 after:rounded-full after:bg-white after:shadow after:transition peer-checked:after:translate-x-6"></span></span></label></div></div></section>
                </div>
            </section>
            ...
            <aside class="space-y-4 sm:space-y-6">
                <section class="admin-side-card w-full rounded-[28px] p-4 sm:rounded-[30px] sm:p-5">
                    ...
                    <div class="mt-5 rounded-[24px] bg-slate-50 px-4 py-4 ring-1 ring-slate-200/70">
                        ...
                        <p class="admin-side-value mt-1 text-sm text-slate-500"><?php echo e($user->email); ?></p>
                        <div class="mt-4 text-sm text-slate-600">
                            <div class="admin-side-row"><span>Téléphone</span><span class="admin-side-value font-semibold text-slate-900"><?php echo e($user->phone ?: 'Non renseigné'); ?></span></div>
                            <div class="admin-side-row"><span>Ville</span><span class="admin-side-value font-semibold text-slate-900"><?php echo e($user->city ?: 'N/A'); ?></span></div>
                            <div class="admin-side-row"><span>Pays</span><span class="admin-side-value font-semibold text-slate-900"><?php echo e($user->country ?: 'N/A'); ?></span></div>
                        </div>
                    </div>
                </section>
                <section class="admin-side-card w-full rounded-[28px] p-4 sm:rounded-[30px] sm:p-5">
                    ...
                    <div class="mt-5 space-y-4">
                        <div>
                            ...
                            <select name="status" id="status" class="admin-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700"><option value="active" <?php if($status === 'active'): echo 'selected'; endif; ?>>Actif</option><option value="suspended" <?php if($status === 'suspended'): echo 'selected'; endif; ?>>Suspendu</option></select>
                        </div>
                        <div class="rounded-[24px] bg-emerald-50 px-4 py-4 ring-1 ring-emerald-200/80">
                            ...
                            <p id="liveBalanceDisplay" class="admin-side-value premium-brand-title mt-3 text-3xl font-semibold text-emerald-700"><?php echo e($balanceFormatted); ?></p>
                        </div>
                        <div>
                            <label for="balance" class="mb-2 block text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Nouveau solde *</label>
                            <input type="text" name="balance" id="balance" value="<?php echo e($balanceInput); ?>" class="admin-field w-full rounded-2xl px-4 py-3 text-sm text-slate-700" required>
                ...
                <section class="admin-side-card w-full rounded-[28px] p-4 sm:rounded-[30px] sm:p-5">
                    ...
                    <div class="mt-5 space-y-3">
                        ...
                        <a href="<?php echo e(localized_route('admin.users')); ?>" class="inline-flex w-full items-center justify-center gap-2 rounded-full border border-slate-200 bg-white px-6 py-3 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50"><i class="fas fa-arrow-left text-xs"></i> Annuler</a>
                    </div>
                </section>
            </aside>
        </div>
...
<?php $__env->startPush('premium_dashboard_scripts'); ?>
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const currencyCode = <?php echo json_encode($user->default_currency ?? 'EUR', 15, 512) ?>;

            phoneInput?.addEventListener('input', function (event) {
                let value = event.target.value.replace(/\D/g, '');
                if (value.startsWith('33')) value = value.replace(/^33/, '0');
                event.target.value = value.length ? value.match(/.{1,2}/g).join(' ') : '';
            });
...
<?php echo $__env->make('layouts.admin-premium', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\cerveau\resources\views\admin\users\edit.blade.php ENDPATH**/ ?>